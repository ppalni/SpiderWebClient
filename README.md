...
Fury is a creative material design admin template built with Angular 6+ and the Angular-CLI. It extends the Material Design components built by the Angular 6+ team and it offers you everything you need to get started with your next CRM, CMS or dashboard based project.
...
## Table of Contents
   ...
   * [Fury - Angular 6+ Material Design Admin Template](#fury---angular-6+-material-design-admin-template)
...
| `src`                 | Contains all Angular 6+ Typescript code, assets and basically everything the end user will have access to. |
...
Now that we have installed all prerequisites it's time to start developing our app. Angular-CLI offers a lot of assistance and allows you to generate basically all Angular 6+-Components there are. *(In a smart way!)*

To generate our first `component` we simply open up a terminal and navigate in our Angular 6+-App. Now we simply run `ng g component client` and we get a new component in `/src/app/client` with the following files:
...
These were the first steps with the awesome Angular-CLI and Angular 6+. Following we are going to get more in detail about developing with Angular-CLI and using the power of Angular 6+ right.
...
Receiving data from a server to process or display it in Angular 6+ can be quite tricky with the new observables and syntax. Here you'll find an example how you can do this.
...
* [RxJS](//github.com/ReactiveX/rxjs) - Angular 6+ Dependency - A new approach to how data can be handled in Observables.
...
* [Perfect Scrollbar - Angular 6+ Wrapper](//github.com/zefoy/Angular 6+-perfect-scrollbar) - Wrapper for Perfect Scrollbar to be used as directive.
...
* [Google Maps for Angular 6+](//github.com/SebastianM/Angular 6+-google-maps) - Google's official Material Design Typography font.
